Shader "Sprites/Default Mix Channels" {
    ...
    SubShader {
        ...
        Pass {
            ...
            float _JustAlphaLerp; // 48 (starting at cb0[3].x)
            float _PreviewLerp; // 52 (starting at cb0[3].y)
            ...
            fout frag(v2f inp)
            {
                ...
                tmp0.w = _EnableExternalAlpha * tmp0.x + tmp1.w;
                tmp0.xyz = float3(1.0, 1.0, 1.0);
                tmp0 = tmp0 * inp.color;
                tmp1.xyz = tmp0.www * tmp0.xyz;
                tmp0.x = 0.0;
                tmp1.w = 1.0 - _PreviewLerp;
                tmp0 = tmp0.xxxw - tmp1;
                tmp0 = _JustAlphaLerp.xxxx * tmp0 + tmp1;
                tmp1.xyz = tmp0.www * float3(0.0, 0.2, 0.2) + -tmp0.xyz;
                tmp1.xyz = _JustAlphaLerp.xxx * tmp1.xyz + tmp0.xyz;
                tmp1.w = tmp0.w;
                tmp0 = tmp0 - tmp1;
                o.sv_target = _PreviewLerp.xxxx * tmp0 + tmp1;
                return o;
            }
