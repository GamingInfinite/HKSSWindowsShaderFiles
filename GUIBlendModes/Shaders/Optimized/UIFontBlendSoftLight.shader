    ...
    SubShader {
        ...
        Pass {
            ...
            fout frag(v2f inp)
            {
                ...
                tmp0.xyz = float3(1.0, 1.0, 1.0) - inp.color.xyz;
                tmp1.xy = inp.texcoord1.xy / inp.texcoord1.ww;
                tmp1.xy = tmp1.xy + float2(1.0, 1.0);
                tmp1.x = tmp1.x * 0.5;
                tmp1.z = -tmp1.y * 0.5 + 1.0;
                tmp1 = tex2D(_GUIBlendingSharedGT, tmp1.xz);
                tmp2.xyz = float3(1.0, 1.0, 1.0) - tmp1.xyz;
                tmp0.xyz = -tmp2.xyz * tmp0.xyz + float3(1.0, 1.0, 1.0);
                tmp2.xyz = tmp1.xyz * tmp2.xyz;
                tmp0.xyz = tmp0.xyz * tmp1.xyz;
                o.sv_target.xyz = tmp2.xyz * inp.color.xyz + tmp0.xyz;
                return o;
            }
            ENDCG
